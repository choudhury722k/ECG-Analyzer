...
from data_structure import Features
...
def batchSignalProcessing(Te=5):
    ...
    for fileName in (range(418, 700)):             # all the mitMVA db files
        for j in (range(2300)):                        # all the episodes
            if (os.path.isfile("Pickles/MITMVAdb/" + str(fileName) + "E" + str(j) + "C" + str(1) + ".p")):

                print("Pickles/MITMVAdb/" + str(fileName) + "E" + str(j) + "C" + str(1) + ".p")
                ...
                processSignal('MITMVA',ecgEpisode.signal,
                              ecgEpisode.Fs, False,
                              label=ecgEpisode.annotation,
                              file=fileName, episode=j, channel=1,Te=Te)
